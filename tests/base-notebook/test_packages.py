...
Example:
    ...
    # 2022-02-17 16:44:36 [    INFO] Running jupyter/base-notebook with args
    #      {'detach': True, 'tty': True, 'command': ['start.sh', 'bash', '-c', 'sleep infinity']} ... (conftest.py:95)
...
def python_packages(packages: dict[str, set[str]]) -> Iterable[str]:
    ...
    return [
        get_package_import_name(package)
        for package in filter(python_package_predicate, packages)
    ]
...
def r_packages(packages: dict[str, set[str]]) -> Iterable[str]:
    ...
    # )
    return [
        get_package_import_name(package[2:])
        for package in filter(r_package_predicate, packages)
    ]
